...
class Admin_Page:
    def __init__(self, top=None):
        ...
        self.button1.configure(text="""تسجيل خروج""")
    ...
    def Logout(self):
        sure = messagebox.askyesno("تسجيل خروج", "هل أنت متأكد أنك تريد تسجيل الخروج؟", parent=adm)
        if sure == True:
            adm.destroy()
            root.deiconify()
            page1.entry1.delete(0, END)
...
class Inventory:
    def __init__(self, top=None):
        ...
        self.message.configure(text="""الاستيراد""")
        self.message.configure(anchor="w")
        ...
        self.button3.configure(font="-family {Poppins SemiBold} -size 12")
        self.button3.configure(borderwidth="0")
        self.button3.configure(text="""أضف منتج""")
    ...
    def search_product(self):
        ...
        try:
            ...
            messagebox.showerror("!عذرا", "رقم المنتج غير صالح", parent=inv)
        else:
            for search in val:
                if search==to_search:
                    self.tree.selection_set(val[val.index(search)-1])
                    self.tree.focus(val[val.index(search)-1])
                    messagebox.showinfo("نجاح!!", "رقم المنتج: {} موجود.".format(self.entry1.get()), parent=inv)
                    break
            else: 
                messagebox.showerror("!عذرا", "رقم المنتج: {} ليس موجود.".format(self.entry1.get()), parent=inv)
...
class Employee:
    ...
    def Logout(self):
        sure = messagebox.askyesno("تسجيل خروج", "هل أنت متأكد أنك تريد تسجيل الخروج؟", parent=emp)
        if sure == True:
            emp.destroy()
            root.deiconify()
            page1.entry1.delete(0, END)
...
def exitt():
    sure = messagebox.askyesno("الخروج","هل أنت متأكد أنك تريد الخروج؟", parent=adm)
    if sure == True:
        adm.destroy()
        root.destroy()
def Exittt():
...
class Buy:
    def __init__(self, top=None):
        ...
        self.button4.configure(text="""اعمل فاتوره""")
        ...
        self.button5.configure(text="""حذف""")
        ...
        self.button8.configure(text="""حذف""")
        ...
        self.button9.configure(text="""ازالة""")
        ...
        if strr.find('Total')==-1:
            ...
            if(product_name!=""):
                ...
                if product_qty.isdigit()==True:
                    if (stock-int(product_qty))>=0:
                        ...
                        messagebox.showerror("!عذرا", "إنتهى من المخزن التحقق من الكمية", parent=by)
                else:
                    messagebox.showerror("عذرا!", "كمية غير صالحة", parent=by)
            else:
                messagebox.showerror("عذرا!", "اختر منتجًا", parent=by)
        else:
            ...
            if(product_name!=""):
                ...
                if product_qty.isdigit()==True:
                    ...
                    messagebox.showerror("عذرا!", "الكمية غير صالحة", parent=by)
            else:
                messagebox.showerror("عذرا!","اختر منتجًا", parent=by)
        ...
        if(self.cart.isEmpty()!=True):
            ...
            if strr.find('Total')==-1:
                try:
                    ...
                    messagebox.showerror("عذرا!", "السلة خاليه", parent=by)
                else:
                    self.Scrolledtext1.configure(state="normal")
            ...
            messagebox.showerror("عذرا!", "أضف منتجًا", parent=by)
    ...
    def gen_bill(self):
        ...
        if self.state == 1:
            ...
            self.wel_bill()
            if(cust_name.get()==""):
                messagebox.showerror("عذرا!", "الرجاء إدخال اسم", parent=by)
            elif(cust_num.get()==""):
                messagebox.showerror("عذرا!", "الرجاء إدخال رقم", parent=by)
            elif valid_phone(cust_num.get())==False:
                messagebox.showerror("عذرا!", "من فضلك أدخل رقما صالحا", parent=by)
                ...
                if strr.find('Total')==-1:
                    ...
                    for name, qty in self.cart.dictionary.items():
                        ...
                        db.commit()
                    messagebox.showinfo("نجاح!!", "تم إنشاء الفاتورة", parent=by)
    ...
    def search_bill(self):
        ...
        if results:
            ...
            messagebox.showerror("خطأ!!", "لم يتم العثور على الفاتورة", parent=by)
            self.entry3.delete(0, END)
...
class Sale_2:
    ...
    def search_product(self):
        ...
        try:
            ...
            messagebox.showerror("!عذرا", "معرف المنتج غير صالح", parent=sa)
        else:
            for search in val:
                if search==to_search:
                    self.tree.selection_set(val[val.index(search)-1])
                    self.tree.focus(val[val.index(search)-1])
                    messagebox.showinfo("نجاح!!", "رقم المنتج: {} موجود.".format(self.entry1.get()), parent=sa)
                    break
            else: 
                messagebox.showerror("!عذرا", "رقم المنتج: {} ليس موجود.".format(self.entry1.get()), parent=sa)
    ...
    def Exit(self):
        sure = messagebox.askyesno("خروج","هل أنت متأكد أنك تريد الخروج؟", parent=sa)
        if sure == True:
            sa.destroy()
            adm.deiconify()
...
class add_product:
    def __init__(self, top=None):
        ...
        self.button1.configure(borderwidth="0")
        self.button1.configure(text="""اضف""")
                            ...
                            try:
                                ...
                                if pmrp:
                                    try:
                                        ...
                                        messagebox.showerror("عذرا!", "سعر بيع غير صالح.", parent=p_add)
                                    else:
                                        if valid_phone(pvendor):
                                            with sqlite3.connect("./Database/store.db") as db:
                                                cur = db.cursor()
                                            insert = (
                                                        "INSERT INTO استيراد(product_name, product_cat, product_subcat, stock, mrp, cost_price, vendor_phn) VALUES(?,?,?,?,?,?,?)"
                                                    )
                                            cur.execute(insert, [pname, pcat, psubcat, int(pqty), float(pmrp), float(pcp), pvendor])
                                            db.commit()
                                            messagebox.showinfo("نجاح!!", "تمت إضافة المنتج بنجاح إلى المخزون", parent=p_add)
                                            p_add.destroy()
                                            page3.tree.delete(*page3.tree.get_children())
                                            page3.DisplayData()
                                            p_add.destroy()
                                        else:
                                            messagebox.showerror("عذرا!", "رقم الهاتف غير صحيح", parent=p_add)
                                else:
                                    messagebox.showerror("عذرا!", "سعر بيع غير صالح.", parent=p_add)
                        else:
                            messagebox.showerror("عذرا!", "الرجاء إدخال سعر تكلفة المنتج", parent=p_add)
                    else:
                        messagebox.showerror("عذرا!", "الرجاء إدخال كمية المنتج", parent=p_add)
                else:
                    messagebox.showerror("عذرا!", "الرجاء إدخال الفئة الفرعية للمنتج", parent=p_add)
            else:
...
def logout():
    sure = messagebox.askyesno("تسجيل الخروج", "هل أنت متأكد أنك تريد تسجيل الخروج؟", parent=by)
    if sure == True:
        by.destroy()
        root.deiconify()
        page1.entry1.delete(0, END)
...
def exitt_1():
    sure = messagebox.askyesno("خروج","هل أنت متأكد أنك تريد الخروج؟", parent=by)
    if sure == True:
        by.destroy()
        root.destroy()
